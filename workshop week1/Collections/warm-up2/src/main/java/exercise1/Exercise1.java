...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys() {
        ...
        Set<String> countriesKeySet = countries.keySet();

        for (String country : countriesKeySet) {
    ...
    public List<String> iteratingOverEntries() {
        ...
        for(Map.Entry<String,String> countryEntry:countriesEntrySet){
            String contryToLower =countryEntry.getKey().toLowerCase();
            if(contryToLower.contains("q")){
                seekingCountries.add(contryToLower);
            }
        }
        return seekingCountries;
    }
    ...
    public String iteratingOverValues() {
        ...
        for(String capital: capitals){
            if(capital.length()>seekingCapital.length()){
